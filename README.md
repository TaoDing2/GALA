...
- **(b)** Detailed schematic of the coupled alignment process, including the unified objective function and optimisation scheme, with convergence illustration. The objective function comprises fidelity terms for transcriptomics and histology (with fusion weight $\alpha$) and a regularisation term penalising the diffeomorphic mapping $\phi^v$. The global stage estimates the affine matrix $A$ via a GA, corresponding to a linear transformation. The local stage jointly refines the diffeomorphic deformation $\phi^v$ and probabilistic matching $P_M$ through an EM-like LDDMM procedure, representing a nonlinear transformation guided by a Gaussian posterior with prior probability $\pi_M$ and covariance matrix $\Sigma_M$. Convergence of fidelity and regularisation across global-local iterations is also illustrated.
